...
performance_profile(hcat([p[:, 1] for p in values(stats)]...),
                                collect(String, [string(s) for s in keys(stats)])) # Profile for #f

# performance_profile(hcat([p[:, 2] for p in values(stats)]...),
#                                 collect(String, [string(s) for s in keys(stats)])) # Profile for #g

# performance_profile(hcat([p[:, 3] for p in values(stats)]...),
#                                 collect(String, [string(s) for s in keys(stats)])) # Profile for #Hv

# performance_profile(hcat([sum(p, 2) for p in values(stats)]...),
#                                 collect(String, [string(s) for s in keys(stats)])) # Profile for #f + #g + #Hv
